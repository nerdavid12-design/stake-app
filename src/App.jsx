          ...
          <div>
            ...
            <div style={{ marginBottom: 40 }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center" }}>
                ...
                {hasBacked && <span className="badge" style={{ background: "#102010", color: "#6fbf6f", border: "1px solid #1a3a1a" }}>You're in</span>}
              </div>
            ...
            <div style={{ background: "#151515", border: "1px solid #2a2a2a", borderRadius: 12, padding: "28px 28px 24px", marginBottom: 28 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 14 }}>
                <div>
                  ...
                  <span style={{ fontSize: 13, color: "#666" }}>of {EVENT.capacity} backed</span>
                </div>
                <div style={{ textAlign: "right" }}>
                  <span style={{ fontSize: 28, fontWeight: 600, color: "#f0ede8", display: "block" }}>${EVENT.depositAmount}</span>
              ...
              <div style={{ marginBottom: 24 }}>
                {EVENT.rewards.map((r, i) => {
                  ...
                  const isNext = !unlocked && (i === 0 || pct >= EVENT.rewards[i - 1].threshold);
                  return (
              ...
              {hasBacked ? (
                ...
                <button className="cta-btn" onClick={handleBack} disabled={animating}>
                  {animating ? "Locking in deposit..." : `Back this show · $${EVENT.depositAmount} deposit`}
                </button>
              )}
        ...
        {view === "venue" && (
          <div>
            <div style={{ marginBottom: 32 }}>
              <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, color: "#f5f0e8", marginBottom: 6 }}>Venue Dashboard</h2>
              <p style={{ fontSize: 14, color: "#666" }}>{EVENT.name} &nbsp;·&nbsp; {EVENT.date}</p>
            </div>

            {/* Stats Grid */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 28 }}>
              {[
                { label: "Backers", value: backers, sub: `of ${EVENT.capacity} capacity`, color: "#e8c97a" },
            ...
            <div className="stat-card">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
                {[
                  { label: "Baseline (flat pricing)", value: `$${EVENT.capacity * EVENT.ticketPrice}` },
                  { label: "Projected with Stake", value: `$${Math.round(EVENT.capacity * EVENT.ticketPrice * (pct >= 0.8 ? 1.32 : pct >= 0.5 ? 1.18 : 1.08))}` },
        ...
        {view === "settlement" && (
          <div>
            <div style={{ marginBottom: 32 }}>
              ...
              <p style={{ fontSize: 14, color: "#666" }}>Simulate what happens when the event closes.</p>
            </div>
                ...
                <div style={{ background: "#102010", border: "1px solid #1a3a1a", borderRadius: 10, padding: "22px 24px", marginBottom: 16 }}>
                  ...
                  <div style={{ display: "grid", gap: 10 }}>
                    {[
                      ...
                      { label: "Venue revenue", action: `$${EVENT.capacity * EVENT.ticketPrice} gross`, detail: `Stake service fee: $${Math.round(EVENT.capacity * EVENT.ticketPrice * 0.10)} (10% uplift share)` },
                    ].map((row, i) => (
                ...
                <div style={{ background: "#1a1508", border: "1px solid #3a3018", borderRadius: 10, padding: "22px 24px", marginBottom: 16 }}>
                  ...
                  <div style={{ display: "grid", gap: 10 }}>
                    {[
                      ...
                      { label: "Venue revenue", action: `$${Math.round(backers * 20 + (EVENT.capacity - backers) * 0.6 * 30)} est. gross`, detail: "Stake service fee calculated on uplift above baseline" },
                    ].map((row, i) => (
            ...
            {/* Backers can simulate adding more to see dashboard change */}
            <div style={{ background: "#151515", border: "1px solid #2a2a2a", borderRadius: 8, padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button
                ...
                onClick={() => { setBackers(b => Math.min(b + 5, EVENT.capacity)); setChartData(d => [...d, { t: "+5", b: Math.min(backers + 5, EVENT.capacity) }]); }}
                disabled={backers >= EVENT.capacity}
              >
                {backers >= EVENT.capacity ? "Sold out" : "+ 5 backers"}
              </button>
            </div>
          </div>
        )}
